...
<script>
    ...
    export default {
        ...
        created: function () {
            ...
            this.w = this.coords.w
            this.h = this.coords.h
            this.elmX = this.coords.x
            this.elmY = this.coords.y
        },
        mounted: function () {
        ...
        watch: {
            ...
            coords({ x, y, w, h }) {
                if ((this.elmX + x >= this.parentX) && (x + this.elmW <= this.parentW)) {
                    this.left = (Math.round( x / this.grid[0] ) * this.grid[0])
                    this.elmX = this.left
                }

                if ((this.elmY + y >= this.parentY) && (y + this.elmH <= this.parentH)) {
                    this.top = (Math.round( y / this.grid[1] ) * this.grid[1])
                    this.elmY = this.top
                }

                if (w > 0 && this.left + w <= this.parentW) {
                    this.width = (Math.round( w / this.grid[0] ) * this.grid[0])
                }

                if (h > 0 && this.elmY + h <= this.parentH) {
                    this.height = (Math.round( h / this.grid[1] ) * this.grid[1])
                }
